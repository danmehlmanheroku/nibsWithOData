...
function addItem(req, res, next) {
    ...
    getPointBalance(userId)
        .then(function(result) {
            ...
            db.query('INSERT INTO interaction (contact_loyaltyid, campaign, product, type, points, name, picture) VALUES ($1, $2, $3, $4, $5, $6, $7)',
...
function getItems(req, res, next) {
    ...
    db.query("SELECT contact_loyaltyid AS userId, campaign AS campaign, type AS type, name as name, picture as picture, points as points, createdDate FROM interaction WHERE contact_loyaltyid=$1 ORDER BY id DESC LIMIT 20", [externalUserId])
...
function deleteItems(userId) {
    ...
    return db.query("DELETE FROM interaction WHERE contact_loyaltyid=$1", [userId]);
}
...
function getPointBalance(userId) {
    return db.query('select sum(points) as points from interaction where contact_loyaltyid=$1', [userId], true);
}
